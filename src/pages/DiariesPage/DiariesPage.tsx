...
import { Button, Cell, FixedLayout, List, Placeholder, Title, Image } from '@telegram-apps/telegram-ui';
...
export const DiariesPage: FC = () => {
  ...
  const {data} = useQuery('diaries', () => getDiaries(initDataRaw))
    ...
    <div>
      <List style={{ paddingBottom: 82, background: 'var(--tgui--bg_color)'}}>
        ...
        {data?.map((diary, i) => <Cell
          ...
          before={<Image size={40} src={''}><Title style={{color: 'var(--tgui--button_color)'}} weight='2' caps>{diary.name[0]}</Title></Image>}
